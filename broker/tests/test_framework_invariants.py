...
class TestNoExcessiveInlineMockPacks:
    ...
    Tests that mock a custom ``DomainPack`` should do so via the
    fixture-package pattern (``examples/_test_fixtures/<domain>/``
    generated by ``broker.tools.gen_test_fixture`` — Phase 6R-A) rather
    than inlining ``class FakePack(DefaultDomainPack)`` boilerplate at
    file scope. Inline mocks couple each test file to the entire
    31-method (soon 7-sub-protocol) Protocol surface; the
    fixture-package pattern isolates the mock into one importable
    package that a future Protocol refactor can update in one place.

    This guard fails if any single test file declares more than 3
    inline subclasses of ``DomainPack`` / ``DefaultDomainPack``.
    Files in the allowlist are exempt — they're the contract-mocking
    files that legitimately need direct subclassing.
    """

    # Allowlist: files that legitimately mock packs at file scope.
    # Reasoning per file:
    # - test_dispatcher_cascade_defense.py — Phase 6Q-D-4 cascade-defense
    #   regression suite; tests the dispatcher's graceful-degradation
    #   path for broken pack methods. Needs broken packs inline by
    #   design (3 today: BrokenFramework, RaisesFramework, RaisesExtreme).
    # - test_cascade_defense_engine_registry.py — Phase 6Q-D-5 sibling
    #   suite for the registry smoke-test cascade path. Needs broken
    #   packs inline (1 today).
    # - test_gen_test_fixture.py — false positives (the file contains
    #   `assert "class XDomainPack(DefaultDomainPack):" in text` string
    #   literals that the AST walker is configured to ignore, but
    #   defensive allowlist for any future grep-based audit).
    # - test_domain_pack_contract.py — canonical DomainPack contract
    #   regression suite; if/when it adds inline mocks they're
    #   intentional.
    ALLOWLIST = frozenset({
        "test_dispatcher_cascade_defense.py",
        "test_cascade_defense_engine_registry.py",
        "test_gen_test_fixture.py",
        "test_domain_pack_contract.py",
    })

    # Both DomainPack and DefaultDomainPack count as "pack bases" — a
    # subclass of either creates an inline mock-pack that we want to
    # encourage moving to a fixture package.
    PACK_BASES = frozenset({"DomainPack", "DefaultDomainPack"})

    # Threshold: > 3 inline classes per file triggers failure. A test
    # file with 1-3 inline mock packs is acceptable (low cognitive
    # cost); above that, the fixture-package pattern is cleaner.
    MAX_INLINE_MOCKS = 3

    def _count_inline_pack_subclasses(self, path):
        """Return the count of class definitions in ``path`` whose
        first base is one of ``PACK_BASES``. AST-based so we don't
        miscount string-literal patterns or commented-out code.

        **Blind spot (acknowledged)**: this catches explicit ``class X(...)``
        definitions only. Dynamic constructions such as
        ``type("X", (DefaultDomainPack,), {})`` or post-construction
        ``__bases__`` mutation slip through. The pattern these dodge is
        rare in practice (no current test file uses it); accept the
        false-negative risk over the engineering cost of a runtime
        instrumentation gate. Phase 6R-A reviewer suggestion #6.
        """
        import ast
        try:
            source = path.read_text(encoding="utf-8")
        ...
        for node in ast.walk(tree):
            ...
            for base in node.bases:
                # Match `class X(DefaultDomainPack)` and
                # `class X(broker.domains.default.DefaultDomainPack)`.
                base_name = None
                if isinstance(base, ast.Name):
                    base_name = base.id
                elif isinstance(base, ast.Attribute):
                    base_name = base.attr
                if base_name in self.PACK_BASES:
                    count += 1
                    break  # one match per class is enough
        return count

    def test_no_test_file_has_more_than_threshold_inline_mock_packs(self):
        """Walk ``broker/tests/`` and ``tests/`` (excluding the
        allowlist) and assert no file exceeds the inline-mock
        threshold. Suggests the ``gen_test_fixture`` pattern."""
        from pathlib import Path

        repo_root = Path(__file__).resolve().parents[2]
        scan_dirs = [repo_root / "broker" / "tests", repo_root / "tests"]
        offenders = []
        for scan_dir in scan_dirs:
            ...
            for path in sorted(scan_dir.rglob("test_*.py")):
                if path.name in self.ALLOWLIST:
                    continue
                count = self._count_inline_pack_subclasses(path)
                if count > self.MAX_INLINE_MOCKS:
                    rel = path.relative_to(repo_root)
                    offenders.append(f"{rel}: {count} inline mock packs")
        assert not offenders, (
            "test files exceeded the inline-mock-pack threshold "
            f"(> {self.MAX_INLINE_MOCKS} subclasses of "
            f"{sorted(self.PACK_BASES)} at file scope).\n"
            "Prefer the fixture-package pattern: generate a reusable "
            "test fixture via\n"
            "    python -m broker.tools.gen_test_fixture "
            "--domain <name> --output examples/_test_fixtures/\n"
            "and import it from the test. Reference: "
