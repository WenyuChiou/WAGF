...
class MemoryEngine(ABC):
    ...
    @abstractmethod
    def retrieve(self, agent: BaseAgent, query: Optional[str] = None, top_k: int = 3, **kwargs) -> List[Dict[str, Any]]:
        """
        Retrieve relevant memories for an agent.

        Returns a list of memory records, each a dict with at least:
            - content (str)
            - emotion (str)
            - importance (float)
            - source (str)
            - timestamp (int)
            - final_score (float, optional)

        Per `broker/INVARIANTS.md` Invariant 1, all subclasses must preserve
        metadata end-to-end. Use `retrieve_content_only()` for legacy
...
from .engines.window import WindowMemoryEngine
from .engines.importance import ImportanceMemoryEngine
from .engines.humancentric import HumanCentricMemoryEngine
from .engines.hierarchical import HierarchicalMemoryEngine
from .seeding import seed_memory_from_agents
