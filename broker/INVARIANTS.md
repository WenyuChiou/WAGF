...
**Deferred** (deliberate scoping decisions from Phase 6K's plan):
- **Phase 6L "knobs"** — 8 magic-constant tuning surfaces from audit-B P1 (`retriever.py` `top_n`/`min_score`, `drift.py` 4 thresholds, `reflection.py` importance constants, `bridge.py` resolution-event importance, `council.py` 0.5 quorum, `cross_agent_validator.py` 3 thresholds). These are config knobs, not boundary issues; wire them to YAML / `DomainPack` separately.
- **Phase 6M "PMT schema extraction"** — `broker/interfaces/schemas.py` `ReasoningSchema` Pydantic fields `threat_appraisal` / `coping_appraisal` → framework-parametric base. Touchable Pydantic class hierarchy + per-caller migration; risk profile higher than 6K's other items. Today's reference domains either already use their own schema (vaccination_demo) or expect the PMT fields (flood / irrigation), so this is a theoretical blocker; deferred until a genuinely non-PMT domain plugs in.
