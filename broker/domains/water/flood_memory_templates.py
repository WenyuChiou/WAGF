...
Phase 6B-2 (2026-05-04): relocated from
`broker/components/prompt_templates/memory_templates.py` because the
contents are wholly flood-domain specific (FEMA, flood_zone, SFHA, NFIP).
The original location now hosts only the generic `MemoryTemplate`
dataclass and the abstract `MemoryTemplateProviderBase`. New domains
should create their own provider here under `broker/domains/<domain>/`.

Originally moved from `examples/multi_agent/memory/templates.py` for
SA/MA reuse.
"""
from typing import Dict, Any, List

from broker.components.prompt_templates.memory_templates import MemoryTemplate


class FloodMemoryTemplateProvider:
    """
    ...
    New domains should create a parallel provider class subclassing
