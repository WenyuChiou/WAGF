...
- **實作方式**：
  ...
  - **衰減函數**：`S = I × e^(-λt)`。創傷性事件因 I 高，在腦中保持「可得性」的時間遠長於日常瑣事。
...
- **實作方式**：
  ...
  - **加權檢索**：`S = Wr×R + Wi×I + Wc×C`。即使是十年前的老記憶，只要環境匹配，就會被重新喚醒。
...
- **實作方式**：
  - **日常模式 (PE < θ)**：執行 v1 (系統一/習慣)。節省算力。
  - **驚奇模式 (PE ≥ θ)**：執行 v2 (系統二/深思)。強制啟動情境感知檢索。
