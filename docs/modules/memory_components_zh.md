...
| **核心公式** | $S(t)=I\cdot e^{-\lambda t}$ | $S=W_rR+W_iI+W_cC+W_{rel}Rel-W_{int}Int$ | $PE=\|Reality-Expectation\|$ | $S=1-P(Signature)$ |
...
| $W_{rel}$ | **相關權重** | 0.0 | 查詢與記憶的關鍵字重疊權重（P2） |
| $W_{int}$ | **干擾權重** | 0.0 | 逆向干擾懲罰權重（P2） |
| $\gamma$ | **干擾上限** | 0.8 | 最大干擾懲罰（保留部分檢索） |
...
v2 使用 $S=(R \times W_r)+(I \times W_i)+(C \times W_c)+(Rel \times W_{rel})-(Int \times W_{int})$。
預設 $W_r{=}0.3,\; W_i{=}0.5,\; W_c{=}0.2,\; W_{rel}{=}0,\; W_{int}{=}0$ 時簡化為經典公式。
...
其中 $\gamma$ 為干擾上限（預設 0.8）。設定 `W_interference > 0` 啟用。
...
- **Unigram 模式**：$Surprise = 1 - P(action \mid history)$，含 Laplace 平滑
- **Bigram 模式**：$Surprise = 1 - P(action \mid prev\_action)$，偵測轉移異常
