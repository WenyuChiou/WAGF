...
class TestUniversalCognitiveEngine(unittest.TestCase):
    def setUp(self):
        ...
        # Mock the parent retrieve to track calls
        self.patcher = patch('broker.components.universal_memory.HumanCentricMemoryEngine.retrieve')
    ...
    def test_system_1_routine_low_surprise(self):
        ...
        engine = UniversalCognitiveEngine(arousal_threshold=2.0, ema_alpha=0.5)
        
        # Reality matches Expectation (0 vs 0) -> Surprise = 0
        state = {'flood_depth': 0.0}
    ...
    def test_normalization_adaptation_cycle(self):
        ...
        Constant flood should eventually become routine (System 2 -> System 1).
        """
        # Alpha=0.5 means fast adaptation for testing
        engine = UniversalCognitiveEngine(arousal_threshold=1.0, ema_alpha=0.5)
        
        # Tick 1: Surprise! (0 vs 10) -> System 2
        state = {'flood_depth': 10.0}
        engine.retrieve(self.agent, world_state=state)
        ...
        self.assertEqual(engine.current_system, "SYSTEM_1", "Agent should have normalized to the flood by now.")
