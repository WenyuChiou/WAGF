...
| _檢定類型_        |        | _卡方檢定 (5x2 全分佈)_ |               |

**行為分佈變化 (Window vs Baseline) [2025 Fix Updated]:**

- **多樣性恢復**: 熵值在洪水年 (Year 4-7) 維持 > 1.2，不再是死板的「不做任何事」。
- **理性收斂 (Rational Convergence)**: 第 8 年熵值下降至 0.48，是因為 **64% 的代理已經完成適應** (抬高或搬遷)，回到「不做任何事」是正確的維護行為。
- **信任動力學**: 對鄰居的信任度從 0.42 逐步上升至 0.72，確認了社會學習機制有效。

**洪水年響應（適應行為）：**

- **Year 3**: 洪水發生 (觸發點)。
- **Year 4**: 抬高房屋 (Elevate) 顯著增加。
- **Year 9**: 大多數代理已處於安全狀態。

**行為洞察 (Updated):**

- **已修正 (Static Bug Resolved)**: 之前 Gemma 因「信任分數重置」導致行為死板。修復 `InteractionHub` 後，模型展現出正常的學習曲線：受災 -> 恐懼 -> 行動 -> 安全。
- **被動合規**: 雖然仍有 0 次拒絕，但這次是因為模型做出了**正確的高效能決策** (如在受災後選擇抬高)，符合治理規則。
...
- Window 記憶減少了 36 次搬遷。模型未長期維持高威脅評估，因此未觸發極端行動。
...
| 模型           | 阻斷事件 | 成功修正 (T1/T2/T3) | 失敗 (3 次重試) | 修正成功率 |
| -------------- | -------- | ------------------- | --------------- | ---------- |
...
| --------------------------------------------- | ---- | ---------------- | ------------- | --------- | -------------------- |
| `elevation_threat_low`                        | 152  | 60               | 92            | **39.5%** | 偏執/頑固 (Stubborn) |
| `relocation_threat_low`                       | 40   | 12               | 28            | **30.0%** | Mixed results.       |
| `relocation_threat_low\|CP_LABEL`             | 17   | 14               | 3             | **82.4%** | 合規 (Compliant)     |
| `relocation_threat_low\|elevation_threat_low` | 5    | 2                | 3             | **40.0%** | 偏執/頑固 (Stubborn) |
