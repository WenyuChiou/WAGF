...
def get_stats(model, group):
    ...
    # 0. JSONL Discovery (for appraisals & interventions)
    jsonl_candidates = [
        group_dir / "raw" / "household_traces.jsonl",
        group_dir / "household_traces.jsonl"
    ]
    # Filter for actually existing files
    jsonl_candidates = [f for f in jsonl_candidates if f.exists()]
    ...
    try:
        ...
        if dec_col:
            ...
            if 'relocated' in full_data.columns:
                ...
                pass # User requested total steps be 1000. Relocated agents are part of the 'N' history until they leave.
                # If we filter them out, we lose the denominator for "Population Survival"
                # But for behavioral statistics (Panic Rate), we only care about ACTIVE agents.
                # Let's verify 'N' definition: User asked "Why is N low?". 
                # Be transparent: Report N_Sim (Total rows) vs N_Audit (Active rows analyzed).
                    ...
                    for line in f:
                        try:
                            ...
                            if retry_active or has_rules:
                                ...
                                # Heuristic: Governance = Rules Failed OR Error is not purely syntax
                                is_syntax = ('json' in parsed_error or 'parse' in parsed_error) and not has_rules
                                
                                # DEBUG for 14B Group C
                                if '14b' in model and 'Group_C' in group and (interv_total < 5):
                                     print(f"   [DEBUG 14B] Found Intv: Retry={retry_active}, Rules={failed_rules[:50]}")
...
for m in models:
    for g in groups:
        ...
        if stats:
            ...
            # V1: Panic Relocation (Freq % + Count)
            v1_pct = f"{stats['V1_%']}%"
            v1_cnt = f"{stats['V1_N']}"
            
            # V2: Panic Elevation (Freq % + Count)
            v2_pct = f"{stats['V2_%']}%"
            v2_cnt = f"{stats['V2_N']}"
            
            # V3: Complacency (Freq % + Count)
            v3_pct = f"{stats['V3_%']}%"
            v3_cnt = f"{stats['V3_N']}"
            
            intv_str = f"{stats['Intv']} ({stats['Intv_S']})"
            
            print(f"{m:<18} {g:<7} {n_str:<6} {v1_pct:<8} {v1_cnt:<6} {v2_pct:<8} {v2_cnt:<6} {v3_pct:<8} {v3_cnt:<6} {intv_str:<12}")
            
            # Collect for Export
            row = stats.copy()
            row['Model'] = m
            row['Group'] = g
            row['Panic_Freq_%'] = row['V1_%']
            row['Panic_Count'] = row['V1_N']
            row['Elevate_Freq_%'] = row['V2_%']
            row['Elevate_Count'] = row['V2_N']
            row['Complacency_%'] = row['V3_%']
            row['Complacency_Count'] = row['V3_N']
            all_data.append(row)
