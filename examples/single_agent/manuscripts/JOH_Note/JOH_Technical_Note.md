...
This design explicitly draws on the "Memory Stream" architecture proposed by **Park et al. (2023)** in _Generative Agents_, but adapts their "Social Salience" metric into a "Survival Salience" model rooted in **Protection Motivation Theory** (**Rogers, 1983**).

#### 2.4.1 Cognitive Process Flow

To simulate the transition from perception to durable belief, we implement a three-stage process. We explicitly couple the **Scoring (Encoding)** and **Retention (Retrieval)** phases based on the **Encoding Specificity Principle** (**Tulving & Thomson, 1973**), which posits that memory recall is dependent on the match between encoding cues and retrieval directives.

1.  **Valuation (Salience)**:
    Every event is assigned a **Unified Importance Score ($I$)**:
    $$I = W_{emotion} \times W_{source}$$
    This calculation simulates the **Construal Level Theory** (**Trope & Liberman, 2010**), where "Personal" experiences (Low Psychological Distance) are weighted significantly higher than "News" (High Psychological Distance).

2.  **Consolidation (Storage)**:
    Memories with $I \geq 0.6$ have a probability ($P$) of being promoted to **Long-Term Memory**, functionally modeling the selective **Episodic Buffer** described by **Baddeley (2000)**:
    $$P_{consolidation} = P_{base} \times I$$

3.  **Retention (Retrieval)**:
    During decision cycles, relevant memories are recalled with a strength governed by the **Forgetting Curve** (**Ebbinghaus, 1885**):
    $$S_{retrieval} = I \times e^{-\lambda t}$$

#### 2.4.2 Emotional Taxonomy (Survival Salience)
...
Tversky, A., & Kahneman, D. (1973). Availability: A heuristic for judging frequency and probability. _Cognitive Psychology_, 5(2), 207-232. - Logic: Validation of high trust/optimism bias overriding risk.
