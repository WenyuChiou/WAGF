...
$GroupAPath = Join-Path $BaseDir "Group_A"
...
Write-Host "[1/3] Launching Group A (Baseline: Ungoverned System 1)..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[2/3] Launching Group B (Governance + Window)..." -ForegroundColor Yellow
Set-Location $ExperimentDir
python -u run_flood.py `
    --model $Model `
    --years $Years `
...
Write-Host ""
Write-Host "[3/3] Launching Group C (Full: Human-Centric + Reflection)..." -ForegroundColor Yellow
Set-Location $ExperimentDir
python -u run_flood.py `
    --model $Model `
    --years $Years `
...
Write-Host "Group A Logs: $GroupAPath"
