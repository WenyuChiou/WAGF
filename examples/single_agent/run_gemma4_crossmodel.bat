...
REM Three sizes: e2b, e4b, 26b — thinking mode disabled via --thinking-mode
REM Each: governed (Group_C) x 1 seed + no-validator (Group_C_disabled) x 1 seed
...
echo   Models: gemma4:e2b, gemma4:e4b, gemma4:26b
...
python examples\single_agent\run_flood.py ^
  --model gemma4:e2b --years 10 --agents 100 --workers 1 ^
  --governance-mode strict --memory-engine humancentric --window-size 5 ^
  --use-priority-schema --thinking-mode disabled ^
...
python examples\single_agent\run_flood.py ^
  --model gemma4:e4b --years 10 --agents 100 --workers 1 ^
  --governance-mode strict --memory-engine humancentric --window-size 5 ^
  --use-priority-schema --thinking-mode disabled ^
  --initial-agents "%PROFILES%" ^
...
python examples\single_agent\run_flood.py ^
  --model gemma4:26b --years 10 --agents 100 --workers 1 ^
  --governance-mode strict --memory-engine humancentric --window-size 5 ^
  --use-priority-schema --thinking-mode disabled ^
  --initial-agents "%PROFILES%" ^
