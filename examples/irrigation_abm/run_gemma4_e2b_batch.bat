...
for %%S in (42 43 44 45 46) do (
    if exist "results\production_v21_42yr_gemma4_e2b_seed%%S\simulation_log.csv" (
        ...
        python run_experiment.py --model gemma4:e2b --years 42 --real --seed %%S --output results/production_v21_42yr_gemma4_e2b_seed%%S --num-ctx 8192 --num-predict 4096
        echo [%date% %time%] DONE: governed gemma4:e2b seed=%%S
    )
    ...
    if exist "results\ungoverned_v21_42yr_gemma4_e2b_seed%%S\simulation_log.csv" (
        ...
        python run_ungoverned_experiment.py --model gemma4:e2b --years 42 --real --seed %%S --output results/ungoverned_v21_42yr_gemma4_e2b_seed%%S --num-ctx 8192 --num-predict 4096
        echo [%date% %time%] DONE: ungoverned gemma4:e2b seed=%%S
    )
)
