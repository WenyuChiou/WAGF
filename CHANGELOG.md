...
- **Generic-framework registry extraction — closes a Phase-6P-A-class leak (pre-6T-F infrastructure)** (2026-05-27). The `utility` and `financial` psychometric-framework METADATA (label orders / constructs / label mappings) lived in `broker/domains/water/thinking_checks.py` even though their construct vocabularies (`BUDGET_UTIL` / `EQUITY_GAP` / `ADOPTION_RATE` for utility; `RISK_APPETITE` / `SOLVENCY_IMPACT` / `MARKET_SHARE` for financial) are genuinely cross-domain. Future `vaccination_ma` (funding-allocation agents) or `traffic_ma` (transit-fare-policy agents) would have to reverse-import from `broker.domains.water` to use them — same anti-pattern Phase 6P-A closed for `validate_all` dispatcher. Triggered by the Phase 6T-F design walkthrough where adding `narrative_diffusion` would have continued the leak. This commit extracts cleanly.
  - **NEW `broker/validators/governance/frameworks/__init__.py`** (~210 LOC): generic-framework home. Defines `UTILITY_LABEL_ORDER` / `UTILITY_CONSTRUCTS` / `UTILITY_LABEL_MAPPINGS` (moved from water), `FINANCIAL_*` (moved from water), and `NARRATIVE_DIFFUSION_*` (NEW). At module-import time calls `register_framework_metadata` for all three. Also imports `NarrativeDiffusionFramework` from sibling module + calls `register_framework("narrative_diffusion", ...)` so the class is in the psychometric registry. Module docstring explains the partial extraction (metadata moved, classes stayed).
  - **NEW `broker/validators/governance/frameworks/narrative_diffusion.py`** (~190 LOC): `NarrativeDiffusionFramework` class — generic narrative-amplification theory. Four constructs (SALIENCE / VIRALITY / AUDIENCE_FIT / NARRATIVE_CONSISTENCY) on 5-point Likert. `get_expected_behavior` returns GENERIC verbs (`amplify` / `counter_narrative` / `share` / `stay_silent`) — domain packs MAP to their concrete skill names (e.g. flood paper-3 `amplify_event` / `post_counter_narrative`). `validate_coherence` warns on the "clickbait signature" (very-high virality + very-low salience). No flood / water / household / examples imports — generic by construction.
  - **`broker/domains/water/thinking_checks.py`** (~50 LOC delta): removed `utility` and `financial` entries from `WATER_FRAMEWORK_LABEL_ORDERS` / `WATER_FRAMEWORK_CONSTRUCTS` / `WATER_LABEL_MAPPINGS`. Kept `pmt` / `dual_appraisal` / `cognitive_appraisal` (genuinely water-specific). Docstring updated to explain the extraction + why CLASS files stayed.
  - **`broker/validators/governance/__init__.py`** (+9 LOC): `from . import frameworks  # noqa: F401` triggers auto-registration at module-load. Every `validate_all` consumer already imports `broker.validators.governance`; piggybacking generic-framework registration here makes it impossible to forget.
  - **`examples/governed_flood/adapters/flood_pack.py:FloodGovernanceMixin._AGENT_TYPE_FRAMEWORK`** (+8 LOC): new entry `"social_media_influencer": "narrative_diffusion"`. Interface-only forward contract — agent_type itself not yet in MA-flood YAML (Phase 6T-F implementation deferred). Inline comment explains the contract.
  - **DECISION: utility.py + financial.py CLASS files stayed in water/** — their `get_expected_behavior` methods return flood-paper-3-coupled skill names (`"increase_subsidy"`, `"reduce_subsidy"` for utility; insurance-domain actions for financial). Calibration tooling at `broker/domains/water/calibration/micro_validator.py:488` consumes these methods. Full extraction would require splitting each Framework into a generic-base + flood-specific subclass — deferred. Only the METADATA (label_order / constructs / mappings) moved this commit. The lazy-import maps in `broker/core/__init__.py:54-55` + `broker/core/psychometric.py:284-285` remain accurate (still point at water/utility.py + water/financial.py paths).
  - **NEW `broker/tests/test_generic_frameworks_registration.py`** (~280 LOC, 12 tests across 4 classes):
    - `TestGenericFrameworksPresent` (3 tests): verify utility / financial / narrative_diffusion are in live `FRAMEWORK_LABEL_ORDERS` + `FRAMEWORK_CONSTRUCTS` after `broker.validators.governance` import.
    - `TestSourceLocationProvesExtraction` (2 tests): inspect water_thinking_checks.py source — utility/financial NOT in WATER_FRAMEWORK_LABEL_ORDERS; verify frameworks/__init__.py exports the three metadata bundles as module-level constants.
    - `TestNarrativeDiffusionFrameworkClass` (5 tests): class registered in psychometric registry; constructs canonical four; 5-point Likert labels; validate_coherence warns on clickbait signature; get_expected_behavior returns generic verbs.
    - `TestFrameworksModuleGenericity` (2 tests via parametrize): AST-scan frameworks/__init__.py + narrative_diffusion.py for forbidden imports (`broker.domains.water`, `examples.*`).
  - **`broker/tests/test_per_agent_framework_dispatch.py`** (+6 LOC): parametrize a 7th case for `social_media_influencer` → `narrative_diffusion`.
  - **Test gate**: `pytest broker/ tests/` → **2731 passed / 10 skipped / 2 xfailed / 0 failed** (+13 vs 2718 baseline; 12 new + 1 parametrize extension). MA flood mock smoke 8/8 green. SA flood mock smoke decision histogram unchanged (Do Nothing: 70 / Insurance: 26 / Elevation: 4 / Both: 0 / Relocate: 0 — happy-path unaffected).
  - **Genericity verified at 3 levels**: live registry + source location + AST scan of new module — matching the protection pattern from Phase 6T-D's `social_media_genericity_audit.md`.
  - **Code-review skill verdict**: APPROVE (HIGH confidence). No P0/P1 findings. P2 self-package-import idiom (`from . import frameworks`) applied pre-commit. P3 stylistic items (late-import grouping, doc redundancy, function vs inline) deferred.
  - **Files changed**: 2 new (frameworks/) + 1 new test + 5 source/test edits + CHANGELOG. ~770 LOC delta net.
  - **What this UNBLOCKS**: Phase 6T-F implementation can now register `social_media_influencer` agent_type in MA-flood YAML + add prompts + lifecycle handler + governance validators, all consuming the `narrative_diffusion` framework that's already in the generic registry. Audit Tension E (narrative_diffusion constructs generic-ness) is closed by this commit — the constructs are genuinely generic and the class lives in a generic home.
