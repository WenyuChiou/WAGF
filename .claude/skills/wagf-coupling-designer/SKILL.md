...
Ask: "Where does your external model live? It it (A) a static file of
...
`.coupling/contract.md` MUST have these sections in this order:

1. `# Coupling contract: <domain> ↔ <external model>` (title)
2. `## Cadence` — agent step, model step, sync points
3. `## Inputs (agent → model)` — table: variable / type / unit /
   range / mapping from skill_id
4. `## Outputs (model → agent context)` — table: variable / type /
   unit / template placeholder / producer code path
5. `## Failure modes` — per-mode planned response
6. `## Units audit checklist` — per-variable check, ticked or
   unchecked
7. `## Mock fidelity` — what the mock preserves vs the real model

This is the same shape `model-coupling-contract-checker` expects as
input. Producing it in this format means the audit pass is
mechanical.
