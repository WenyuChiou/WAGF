...
After all four edits have clean validator output, run the mock smoke from the repo root:

```bash
python examples/<domain>_demo/run_experiment.py --model gemma3:4b --years 2 --agents 3
```

Verify the audit CSV has `years * agents` rows, no `[Adapter:Error]` blocks appeared in stdout, and every `proposed_skill` is one of the declared skill IDs.
