...
> Data source: production_v20_42yr traces (years 1-27, 78 agents).
> Status: governance behavior evidence is complete; CRSS trajectory claims deferred until full simulation_log.csv.
...
[P4] Rule-frequency diagnostics reveal that intervention burden concentrates on hydrologically meaningful constraints. The most frequently triggered rule is `demand_ceiling_stabilizer` (n=923), followed by `high_threat_high_cope_no_increase` (n=701) and `curtailment_awareness_check` (n=177), indicating that governance increasingly enforces feasibility boundaries under chronic shortage conditions. Cluster differentiation remains visible in governed outcomes and is qualitatively consistent with original FQL cluster behavior (Hung and Yang, 2021, Figure 7). Despite high rejection pressure, approved-action diversity remains substantial: normalized entropy over five actions yields H_norm = 0.74, supporting the claim that WAGF constrains implausible behaviors without collapsing the behavioral repertoire.

[P5] The irrigation case validates the metric framework introduced in Section 3. Infeasible proposals (e.g., increasing demand at allocation cap, decreasing below minimum utilization) contribute to feasibility hallucination rate R_H. Coherence failures that remain technically feasible (e.g., high scarcity assessment with high adaptive capacity selecting increase) are tracked as rationality deviation R_R through thinking-rule ERROR traces, enabling cross-domain comparison of governance performance without conflating infeasibility and bounded-rational behavior.

[FIGURE X placeholder: (a) Governance outcome proportions over years; (b) Top rule-trigger composition; (c) Aggregate demand vs CRSS baseline (deferred until full simulation_log.csv)]

---

## Notes for revision

1. **P3-P4 numbers** are from v20 traces (years 1-27). Will update when 42-year run completes.
2. **Demand trajectory** (Figure panel c) requires full `simulation_log.csv` — currently still running.
3. **P1** uses "five graduated skills" which is accurate (v17+ expansion). README confirms the 5-skill design.
4. **P2** "demand corridor as institutional equivalent of FQL reward" is the key conceptual claim. Supported by irrigation_validators.py comments at line 748-751.
5. **v19 vs v20 comparison** available if needed (v19: 70.6% approved, v20: 58.9% — difference explained by demand_ceiling_stabilizer activation in v20).
