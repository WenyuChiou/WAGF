...
const doc = new Document({
  ...
  sections: [{
    ...
    headers: { default: new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: "SAGE: Governance Middleware for LLM-Driven Water ABMs", font: FONT, size: 18, italics: true, color: "666666" })] })] }) },
    ...
    children: [
      // ═══ TITLE ═══
      new Paragraph({ spacing: { after: 120, line: LS }, alignment: AlignmentType.CENTER, children: [new TextRun({ text: "SAGE: A Governance Middleware for LLM-Driven Agent-Based Models of Human\u2013Water Systems", font: FONT, size: 32, bold: true })] }),
      ...
      // ═══ KEY POINTS ═══
      h1("Key Points"),
      new Paragraph({ numbering: { reference: "kp", level: 0 }, spacing: { line: LS }, children: [n("SAGE eliminates 33% hallucination rate in ungoverned LLM agents while preserving genuine behavioral diversity")] }),
      ...
      // ═══ 7. CONCLUSIONS ═══
      h1("7. Conclusions"),
      p([n("We have presented SAGE, the first governance middleware for LLM-driven agent-based models of human\u2013water systems, along with the Effective Behavioral Entropy (EBE) metric for measuring genuine decision diversity. Three findings emerge from our two-domain validation.")], { indent: true }),
      ...
      p([n("Second, governance enables rather than constrains diversity. SAGE-governed agents achieve 32% higher EBE than ungoverned agents, and their cumulative relocation rates (32\u201337%) far exceed the ungoverned baseline (0%), demonstrating that physically valid decision spaces produce richer behavioral outcomes.")], { indent: true }),
      p([n("Third, SAGE transfers across domains without core code changes. The same governance engine serves both flood adaptation (PMT appraisal, 5 skills, 100 agents) and irrigation management (dual appraisal, 5 skills, 78 districts) through configuration-only instantiation, supporting the vision of reusable infrastructure for socio-hydrological ABMs (Giuliani et al., 2022; Castelletti et al., 2010).")], { indent: true }),
      p([n("Future work should address multi-agent interaction governance (agents currently decide independently), dynamic rule learning (governance rules are currently static), and integration with larger basin-scale models (Berglund, 2015). SAGE, the EBE metric, and all experiment code are available open-source at [GitHub URL].")], { indent: true }),
