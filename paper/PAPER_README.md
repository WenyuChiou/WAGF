...
## Paper Workspace Boundary

The `paper/` tree is manuscript-facing working space.
It supports drafting, figure assembly, SI preparation, and paper-specific analysis notes.

Boundary:

- Keep framework-facing onboarding in top-level `README.md` and `docs/guides/`
- Keep manuscript-specific workflow notes in `paper/`
- Do not treat `paper/` as the public framework entry path for ABM developers
...
This `paper/` directory is manuscript-facing, not framework-facing.

Use it for:

- draft writing
- figure/table assembly
- submission packaging
- reviewer responses

Do not treat `paper/` as the primary onboarding path for external ABM developers. The framework-facing surface lives in `broker/`, `docs/guides/`, and the maintained `examples/` reference packs.
